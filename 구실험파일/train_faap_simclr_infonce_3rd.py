"""
FAAP Training - Gender-Aware Score-Based Contrastive Learning (3rd Version)

=============================================================================
2nd 버전 실패 분석:
=============================================================================
- Score 기반 분리가 성별과 무관하게 작동
- obj_score_f ≈ obj_score_m → Low/High 그룹에 여성/남성 균등 분포
- 결과: 성별 공정성 개선 효과 없음 (AP Gap 오히려 악화)

=============================================================================
3rd 버전 핵심 아이디어: Gender-Aware Score-Based Contrastive
=============================================================================

[핵심 변경]
- 2nd: Score만으로 Low/High 분리 (성별 무시)
- 3rd: Gender + Score 결합
  - Anchor: 여성 이미지 (Female)
  - Positive: 남성 중 고성능 이미지 (Male with high score)
  - Negative: 다른 여성 이미지 (Other females)

[수식]
L = -log(exp(sim(z_f, z_m_high)/τ) / [exp(sim(z_f, z_m_high)/τ) + Σ exp(sim(z_f, z_f')/τ)])

[Adaptive Weighting]
- Score 차이에 비례한 가중치: w = 1 + α * (score_m - score_f)
- 여성이 저성능일수록, 남성이 고성능일수록 더 강하게 당김
...
# =============================================================================
# Gender-Aware Score-Based Contrastive Loss (3rd: 핵심 변경)
# =============================================================================

class GenderAwareContrastiveLoss(nn.Module):
    """
    Gender + Score 결합 대조학습

    핵심 변경 (2nd → 3rd):
    - 2nd: Score만으로 Low/High 분리 (성별 무관)
    - 3rd: 여성(Anchor) → 남성 고성능(Positive) 명시적 연결

    수식:
    L = -log(exp(sim(z_f, z_m)/τ) / [exp(sim(z_f, z_m)/τ) + Σ exp(sim(z_f, z_f')/τ)])

    Adaptive Weighting:
    - Score 차이에 비례한 가중치 적용
    - 여성 저성능 ↔ 남성 고성능 쌍에 더 강한 학습
    """
    ...
    def forward(
        ...
        # =================================================================
        # 여성(Anchor) → 남성(Positive), 다른 여성(Negative)
        ...
        # Normalize score_diff to [0, 2] range, then shift to [0.5, 1.5]
        ...
        # =================================================================
        # 4. (Optional) Male → Female 역방향 (약하게)
        # =================================================================
        if n_m >= 2:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("FAAP Gender-Aware Score-Based Contrastive (3rd)")
...
def main():
    ...
    if utils.is_main_process():
        ...
        print("=" * 70)
        print("Gender-Aware Score-Based Contrastive Learning (3rd Version)")
        print("=" * 70)
        print("[2nd 실패 원인] Score 분리가 성별과 무관")
        print("[3rd 핵심 변경] Gender + Score 명시적 결합")
        print("  - Anchor: 여성 (Female)")
        print("  - Positive: 남성 (Male)")
        print("  - Negative: 다른 여성 (Other females)")
        print("  - Adaptive: Score 차이에 비례한 가중치")
        print("-" * 70)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for samples, targets, genders in metrics_logger.log_every(
            ...
            # =================================================================
            # 1. Gender-Aware Contrastive Loss (핵심)
            # =================================================================
            # 이미지 단위 score 계산
    ...
    if utils.is_main_process():
        print("\n" + "=" * 70)
        print("Gender-Aware Score-Based Contrastive Learning Complete!")
        print("=" * 70)
        print(f"Output: {output_dir}")
        print("\n[2nd → 3rd 핵심 변경]")
        print("  - Score만 분리 → Gender + Score 명시적 결합")
        print("  - Anchor: 여성, Positive: 남성")
        print("  - Adaptive weighting: Score 차이 비례 가중치")
        print("\n성공 기준:")
