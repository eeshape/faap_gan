...
def parse_args() -> argparse.Namespace:  # 커맨드라인 인자를 파싱해 Namespace로 반환
    ...
    parser.add_argument("--epsilon_final", type=float, default=0.10, help="target epsilon after warmup")  # 워밍업 후 목표 epsilon
    ...
    parser.add_argument("--beta", type=float, default=0.5, help="detection-preserving loss weight")  # 검출 손실 가중치
    ...
    # 4th 변경: lambda_w 가중치를 0.1 -> 0.2로 상향 (단방향 손실이므로 더 강하게)
    parser.add_argument(
        "--lambda_w",
        type=float,
        default=0.2,
