"""
FAAP Training - Score-Weighted Contrastive + Male Stability (fix10gpu_scf)

Idea 2: Distribution-Fixed Loss
"Female feature가 Male feature 분포를 닮아라 (Gaussian kernel weighted, Male detach)"

=============================================================================
fix8 vs fix10 실험 비교:
=============================================================================
fix8_women: Quality bin 기반 SupCon + Wasserstein (Gender-Centered)
fix10_scf:  Gaussian kernel weighted contrastive + Male Stability Loss

=============================================================================
핵심 변경사항 (fix8 → fix10):
=============================================================================
1. GenderAnchorSupConLoss → ScoreWeightedContrastiveLoss
   - Quality bin 제거 (torch.quantile, torch.bucketize 없음)
   - Gaussian kernel weight: w(a,p) = exp(-|s_a - s_p|² / 2σ²)
   - σ = std(scores) * bandwidth_scale
   - Male(target) projection은 .detach() (단방향 gradient)

2. Male Feature Stability Loss 추가
   - 원본(비섭동) DETR features vs 섭동 DETR features의 MSE
   - Male에만 적용: perturbation이 Male feature를 변화시키지 않도록

3. Wasserstein loss 완전 제거
   - _wasserstein_1d_unidirectional() 삭제
   - _resize_sorted() 삭제

4. 총 손실:
   total_g = lambda_con * contrastive_weight * loss_contrastive
           + lambda_stab * loss_stability
           + current_beta * loss_det

=============================================================================
Anchor 설정:
=============================================================================
Anchor:   Female 샘플 (변해야 하는 쪽)
Target:   Male 샘플 (목표, .detach())
Weight:   Gaussian kernel (score 차이 기반)
...
# =============================================================================
# Score-Weighted Contrastive Loss (Anchor = Female, Target = Male detach)
# =============================================================================

class ScoreWeightedContrastiveLoss(nn.Module):
    """
    Score-Weighted Contrastive Loss with Gaussian kernel.

    Anchor: Female만 (anchor_gender="female")
    Target: Male (target_gender="male"), projection은 .detach()로 단방향 gradient
    Weight: Gaussian kernel w(a,p) = exp(-|s_a - s_p|² / 2σ²)
            σ = std(scores) * bandwidth_scale

    Female feature → Male feature 분포를 닮아가도록 학습.
    Male은 gradient를 받지 않음 (unidirectional).
    """
    ...
    def forward(
        ...
        # Male projection을 detach: 단방향 gradient (Female만 업데이트)
        proj_anchor = projections[anchor_t]            # (n_a, D), with grad
        proj_target = projections[target_t].detach()   # (n_t, D), no grad

        scores_anchor = scores[anchor_t]   # (n_a,)
        scores_target = scores[target_t]   # (n_t,)

        # Gaussian kernel bandwidth: σ = std(all scores) * bandwidth_scale
        score_std = scores.detach().std().clamp(min=1e-6)
        sigma = score_std * self.bandwidth_scale

        # Gaussian kernel weights: w(a, p) = exp(-|s_a - s_p|² / 2σ²)
        # (n_a, n_t) 행렬
        score_diff_sq = (scores_anchor.unsqueeze(1) - scores_target.unsqueeze(0)) ** 2  # (n_a, n_t)
        weights = torch.exp(-score_diff_sq / (2 * sigma ** 2))  # (n_a, n_t)

        # 가중 positive similarity: anchor(Female) vs target(Male)
        sim_a2t = torch.mm(proj_anchor, proj_target.t()) / self.temperature  # (n_a, n_t)

        # Denominator: logsumexp over all non-self pairs
        # Anchor(Female) vs 전체 (anchor + target, self 제외)
        proj_all = torch.cat([proj_anchor, proj_target], dim=0)  # (n_a + n_t, D)
        sim_a2all = torch.mm(proj_anchor, proj_all.t()) / self.temperature  # (n_a, n_a+n_t)

        # Self mask: anchor의 자기 자신 제외
        mask_self = torch.zeros(len(anchor_idx), len(anchor_idx) + len(target_idx),
                                device=projections.device, dtype=torch.bool)
        for i in range(len(anchor_idx)):
            mask_self[i, i] = True
        sim_a2all_masked = sim_a2all.masked_fill(mask_self, float('-inf'))
        log_denom = torch.logsumexp(sim_a2all_masked, dim=1)  # (n_a,)

        # Weighted numerator: Σ_p w(a,p) * sim(a,p) / Σ_p w(a,p)
        w_sum = weights.sum(dim=1).clamp(min=1e-6)  # (n_a,)
        weighted_sim = (sim_a2t * weights).sum(dim=1) / w_sum  # (n_a,)

        # InfoNCE-style loss per anchor
        per_anchor_loss = -(weighted_sim - log_denom)
        loss = per_anchor_loss.mean()

        # 로깅
        score_gap = (scores_target.mean() - scores_anchor.mean()).item()

        info = {
            "n_anchor": len(anchor_idx),
            "n_target": len(target_idx),
            "score_gap": score_gap,
            "score_f_mean": scores_anchor.mean().item(),
            "score_m_mean": scores_target.mean().item(),
...
def parse_args():
    parser = argparse.ArgumentParser("FAAP fix10 scf (Score-Weighted Contrastive + Male Stability)")
    ...
    # fix10: lambda_supcon/lambda_wass/n_bins 제거, lambda_con/lambda_stab/bandwidth_scale 추가
    parser.add_argument("--lambda_con", type=float, default=1.0)
    parser.add_argument("--lambda_stab", type=float, default=0.5)
...
def main():
    ...
    if utils.is_main_process():
        ...
        print("=" * 70)
        print("Score-Weighted Contrastive + Male Stability (fix10gpu_scf)")
        print("=" * 70)
        print("[Anchor 설정]")
        print("  Anchor:   Female (변해야 하는 쪽)")
        print("  Target:   Male (목표, .detach() → 단방향 gradient)")
        print("  Weight:   Gaussian kernel w(a,p) = exp(-|s_a - s_p|² / 2σ²)")
        print("  Stability: Male feature MSE (원본 vs 섭동)")
        print("  → Female feature가 Male feature 분포를 닮아가도록 학습")
        print("-" * 70)
        print(f"Temperature: {args.temperature}, Bandwidth scale: {args.bandwidth_scale}")
        print(f"lambda_con: {args.lambda_con}, lambda_stab: {args.lambda_stab}")
    ...
    contrastive_loss_fn = ScoreWeightedContrastiveLoss(args.temperature, args.bandwidth_scale).to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for samples, targets, genders in metrics_logger.log_every(train_loader, args.log_every, f"Epoch {epoch}"):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                # --- Stability Loss: 원본 DETR features (Male, no grad) ---
                with torch.no_grad():
                    outputs_orig, features_orig = detr.forward_with_features(samples)
                    # features_orig: (B, num_queries, hidden_dim)
                    features_orig_pooled = features_orig.mean(dim=1)  # (B, hidden_dim)
                ...
                features_pert_pooled = features.mean(dim=1)  # (B, hidden_dim)
                ...
                # --- Contrastive Loss (ScoreWeightedContrastiveLoss) ---
                projections = proj_head(features)
                loss_contrastive, con_info = contrastive_loss_fn(projections, image_scores, genders)

                # --- Male Feature Stability Loss ---
                loss_stability = perturbed_tensors.new_tensor(0.0)
                if len(male_idx) > 0:
                    male_t = torch.tensor(male_idx, device=device, dtype=torch.long)
                    loss_stability = F.mse_loss(
                        features_pert_pooled[male_t],
                        features_orig_pooled[male_t].detach(),
                    )
                ...
                total_g = (
                    args.lambda_con * contrastive_weight * loss_contrastive
                    + args.lambda_stab * loss_stability
                    + current_beta * loss_det
                )
            ...
            metrics_logger.update(
                loss_contrastive=loss_contrastive.item(), loss_stability=loss_stability.item(),
                ...
                score_f=con_info.get("score_f_mean", 0.0),
                score_m=con_info.get("score_m_mean", 0.0),
                valid_anchors=con_info.get("valid_anchors", 0),
        ...
        if utils.is_main_process():
            ...
            log_entry = {
                ...
                "loss_stability": metrics_logger.meters["loss_stability"].global_avg,
            ...
            print(f"\n[Epoch {epoch}] Anchor=FEMALE, Target=MALE (detach)")
            print(f"  Contrastive: {log_entry['loss_contrastive']:.4f} (valid_anchors: {log_entry['valid_anchors']:.0f})")
            print(f"  Stability (Male): {log_entry['loss_stability']:.4f}")
    ...
    if utils.is_main_process():
        print(f"\n{'='*70}")
        print(f"Training Complete — Score-Weighted Contrastive + Male Stability (fix10gpu_scf)")
