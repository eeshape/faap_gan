"""
FAAP Training - Fair Centroid Contrastive Learning (3rd_fix5_gpu) [2026-03-05]

=============================================================================
3rd_fix1 → 3rd_fix5 변경사항:
=============================================================================

[핵심 변경 1: Score 계산 방식]
- 기존 (fix1): _image_level_detection_score (top-k max prob, image-level)
  → confidence score != AP 불일치 문제 지속
- 수정 (fix5): _matched_detection_scores (Hungarian matching 기반 TP confidence)
  → AP와 직접 연관된 matched prediction의 confidence만 추출
  → AP Gap 직접 공략 가능

[핵심 변경 2: Wasserstein 방향]
- 기존 (fix1): _wasserstein_1d_symmetric (양방향 |F-M|)
  → 남성 score도 낮출 수 있어 전체 성능 하락 위험
- 수정 (fix5): _wasserstein_1d_asymmetric (단방향 ReLU(M-F))
  → 여성만 남성 수준으로 끌어올림 (7th 버전 전략 계승)
  → 남성 성능 유지 + 여성만 향상
...
=============================================================================
_gpu 버전 추가 변경: A100 BF16 AMP
=============================================================================
- torch.autocast(dtype=torch.bfloat16) 적용 (GradScaler 불필요)
- TF32 활성화 (A100 기본값 명시)
- batch_size 기본값 16으로 상향 (메모리 절감 활용)
=============================================================================
"""
...
class FairCentroidContrastiveLoss(nn.Module):
    ...
    def _update_centroids(self, proj_f: torch.Tensor, proj_m: torch.Tensor):
        ...
        current_f = proj_f.float().mean(dim=0)
        current_m = proj_m.float().mean(dim=0)
    ...
    def forward(
        ...
        fair_centroid = self.fair_centroid.to(proj_f.dtype).unsqueeze(0)  # (1, D)
...
# =============================================================================
# [fix5 신규] Matched Detection Score (Hungarian matching 기반)
# =============================================================================
...
def _matched_detection_scores(
    ...
    AP는 matched prediction의 confidence rank에 민감하므로,
    image-level score보다 AP에 직접적으로 연관된 score를 사용.

    Returns:
        1D tensor of confidence scores for matched (TP) predictions.
        빈 텐서 반환 시 fallback 로직에서 image-level score 사용.
    """
    if len(targets) == 0:
        return outputs["pred_logits"].new_zeros(0)

    indices = detr.criterion.matcher(outputs, targets)
    probs = outputs["pred_logits"].softmax(dim=-1)  # (B, Q, C+1)
...
# =============================================================================
# [fix5 신규] Asymmetric Wasserstein Loss (단방향)
# =============================================================================

def _resize_sorted(scores: torch.Tensor, k: int) -> torch.Tensor:
    """선형 보간으로 크기 조정"""
    if scores.numel() == k:
        return scores
    idx = torch.linspace(0, scores.numel() - 1, k, device=scores.device)
    idx_low, idx_high = idx.floor().long(), idx.ceil().long()
    weight = idx - idx_low
...
def _wasserstein_1d_asymmetric(
    ...
    """단방향 Wasserstein: 여성 score를 남성 수준으로 끌어올림.

    F.relu(M - F): 여성 < 남성일 때만 패널티 → 남성 성능 유지
    male_scores.detach(): 남성은 타겟으로 고정 (gradient 차단)
    """
    if female_scores.numel() == 0 or male_scores.numel() == 0:
        return female_scores.new_tensor(0.0)

    sorted_f = female_scores.sort().values
    sorted_m = male_scores.detach().sort().values  # 남성은 타겟으로 고정
...
def main():
    ...
    if utils.is_main_process():
        ...
        print("=" * 70)
        print("Fair Centroid Contrastive Learning (3rd_fix1_gpu)")
        print("=" * 70)
        print("[3rd 실패 원인]")
        ...
        print("-" * 70)
        print("[_gpu 추가 변경]")
        print("  1. BF16 AMP (torch.autocast, GradScaler 불필요)")
        print("  2. TF32 활성화 (A100 Tensor Core 최대 활용)")
        print("  3. batch_size 기본값 16 (메모리 절감 → 배치 확대)")
        print("-" * 70)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for samples, targets, genders in metrics_logger.log_every(
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                # =============================================================
                # 2. Asymmetric Wasserstein Loss [fix5: matched scores 사용]
                # =============================================================
                # outputs를 성별별로 슬라이스하여 Hungarian matcher에 전달
                outputs_f = {"pred_logits": outputs["pred_logits"][female_idx],
                             "pred_boxes": outputs["pred_boxes"][female_idx]}
                outputs_m = {"pred_logits": outputs["pred_logits"][male_idx],
                             "pred_boxes": outputs["pred_boxes"][male_idx]}
                targets_f = [targets[i] for i in female_idx]
                targets_m = [targets[i] for i in male_idx]

                matched_scores_f = _matched_detection_scores(detr, outputs_f, targets_f)
                matched_scores_m = _matched_detection_scores(detr, outputs_m, targets_m)

                # matched score가 부족하면 image-level score로 fallback
                if matched_scores_f.numel() < 2 or matched_scores_m.numel() < 1:
                    loss_wasserstein = _wasserstein_1d_asymmetric(scores_f, scores_m)
                    wass_mode = "fallback_img"
                else:
    ...
    if utils.is_main_process():
        print("\n" + "=" * 70)
        print("Fair Centroid Contrastive Learning Complete! (3rd_fix1_gpu)")
        print("=" * 70)
        ...
        print("\n[_gpu 추가 변경]")
        print("  - BF16 AMP (torch.autocast)")
        print("  - TF32 활성화")
        print("  - batch_size 기본값 16")
        print("\n성공 기준:")
