...
def parse_args() -> argparse.Namespace:  # 커맨드라인 인자를 파싱해 Namespace로 반환
    parser = argparse.ArgumentParser(
        "FAAP-style training for DETR (10th version - Adaptive Alignment + Baseline Uplift)",
        add_help=True
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="adaptive",
        choices=["adaptive", "female_to_male"],
        help="score alignment direction (adaptive = pull lower group toward higher)",
    )
    parser.add_argument(
        "--gap_scale_temp",
        type=float,
        default=0.05,
        help="temperature for gap-based scaling of alignment losses (0 disables scaling)",
    )
    parser.add_argument(
        "--lambda_det_guard",
        type=float,
        default=0.7,
        help="weight for per-gender detection regression guard vs baseline",
    )
    parser.add_argument(
        "--det_guard_margin",
        type=float,
        default=0.0,
        help="allowed detection loss regression before guard penalty",
    )
    parser.add_argument(
        "--lambda_uplift",
        type=float,
        default=0.3,
        help="weight for baseline score uplift loss (both genders)",
    )
    parser.add_argument(
        "--uplift_margin",
        type=float,
        default=0.02,
        help="margin to enforce score uplift over baseline",
    )
    parser.add_argument(
        "--uplift_weight_f",
        type=float,
        default=1.0,
        help="female uplift scaling factor",
    )
    parser.add_argument(
        "--uplift_weight_m",
        type=float,
        default=1.0,
        help="male uplift scaling factor",
    )
...
def _wasserstein_1d(source_scores: torch.Tensor, target_scores: torch.Tensor) -> torch.Tensor:
    """단방향 Wasserstein 손실: source score가 target보다 낮을 때만 패널티.
    
    7th에서 검증된 핵심 메커니즘 유지:
    - source < target일 때만 손실 발생
    - target은 detach로 타겟 역할 (보호)
    """
    if source_scores.numel() == 0 or target_scores.numel() == 0:
        return source_scores.new_tensor(0.0, device=source_scores.device)
    sorted_s = source_scores.sort().values
    sorted_t = target_scores.detach().sort().values  # target은 detach로 타겟 역할
    k = max(sorted_s.numel(), sorted_t.numel())
    sorted_s = _resize_sorted(sorted_s, k)
    sorted_t = _resize_sorted(sorted_t, k)
    # 단방향: source가 target보다 낮을 때만 손실 발생 (source → target 방향 이동)
    return F.relu(sorted_t - sorted_s).mean()
...
def _quantile_matching_loss(
    ...
    Args:
        source_scores: 정렬 대상 그룹의 detection confidence scores
        target_scores: 타겟 그룹의 detection confidence scores  
    ...
    q_target = torch.quantile(target_scores.detach(), quantile_levels)  # target은 타겟
...
def _score_gap_penalty(
    ...
    단방향: source 평균이 target보다 낮을 때만 손실 발생
    """
    if source_scores.numel() == 0 or target_scores.numel() == 0:
        return source_scores.new_tensor(0.0, device=source_scores.device)
    
    mean_s = source_scores.mean()
    mean_t = target_scores.detach().mean()  # target은 타겟
    
    # 단방향: source 평균이 target보다 낮을 때만 패널티
    return F.relu(mean_t - mean_s)


def _score_uplift_loss(
    pert_scores: torch.Tensor,
    base_scores: torch.Tensor,
    margin: float,
) -> torch.Tensor:
    """Baseline 대비 score uplift 손실."""
    if pert_scores.numel() == 0 or base_scores.numel() == 0:
        return pert_scores.new_tensor(0.0, device=pert_scores.device)
    pert_mean = pert_scores.mean()
    base_mean = base_scores.detach().mean()
    return F.relu(base_mean + margin - pert_mean)
...
def _select_alignment_pair(
    ...
    if female_scores.numel() == 0 or male_scores.numel() == 0:
        zero = female_scores.new_tensor(0.0, device=female_scores.device)
        return None, None, zero, zero
    mean_f = female_scores.mean()
    mean_m = male_scores.mean()
    if align_mode == "female_to_male":
        gap = F.relu(mean_m - mean_f)
        align_dir = female_scores.new_tensor(1.0)
        return female_scores, male_scores, gap, align_dir
    # adaptive: lower group → higher group
    if mean_f <= mean_m:
        gap = mean_m - mean_f
        align_dir = female_scores.new_tensor(1.0)
        return female_scores, male_scores, gap, align_dir
    gap = mean_f - mean_m
    align_dir = female_scores.new_tensor(-1.0)
    return male_scores, female_scores, gap, align_dir


def _gap_scale(gap_value: torch.Tensor, temp: float) -> torch.Tensor:
    if temp <= 0:
        return gap_value.new_tensor(1.0, device=gap_value.device)
    return (gap_value / (gap_value + temp)).clamp(0.0, 1.0).detach()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for samples, targets, genders in metrics_logger.log_every(train_loader, args.log_every, f"Epoch {epoch}"):
            ...
            if female_batch is not None or male_batch is not None:
                ...
                det_guard = torch.tensor(0.0, device=device)
                det_f = torch.tensor(0.0, device=device)
                det_m = torch.tensor(0.0, device=device)
                det_f_base = torch.tensor(0.0, device=device)
                det_m_base = torch.tensor(0.0, device=device)
                uplift_loss = torch.tensor(0.0, device=device)
                uplift_f = torch.tensor(0.0, device=device)
                uplift_m = torch.tensor(0.0, device=device)
                gap_value = torch.tensor(0.0, device=device)
                gap_scale = torch.tensor(0.0, device=device)
                align_dir = torch.tensor(0.0, device=device)
                total_g = torch.tensor(0.0, device=device)

                female_scores = torch.tensor([], device=device)
                male_scores = torch.tensor([], device=device)
                female_scores_base = torch.tensor([], device=device)
                male_scores_base = torch.tensor([], device=device)

                if args.lambda_det_guard > 0 or args.lambda_uplift > 0:
                    with torch.no_grad():
                        if female_batch is not None:
                            outputs_f_base, _ = detr.forward_with_features(female_batch)
                            det_f_base, _ = detr.detection_loss(outputs_f_base, female_targets)
                            if args.lambda_uplift > 0:
                                female_scores_base = _matched_detection_scores(detr, outputs_f_base, female_targets)
                        if male_batch is not None:
                            outputs_m_base, _ = detr.forward_with_features(male_batch)
                            det_m_base, _ = detr.detection_loss(outputs_m_base, male_targets)
                            if args.lambda_uplift > 0:
                                male_scores_base = _matched_detection_scores(detr, outputs_m_base, male_targets)
                ...
                det_loss = det_f + det_m
                if args.lambda_det_guard > 0:
                    det_guard_f = F.relu(det_f - det_f_base + args.det_guard_margin) if female_batch is not None else det_f
                    det_guard_m = F.relu(det_m - det_m_base + args.det_guard_margin) if male_batch is not None else det_m
                    det_guard = det_guard_f + det_guard_m
                if args.lambda_uplift > 0:
                    uplift_f = (
                        _score_uplift_loss(female_scores, female_scores_base, args.uplift_margin)
                        if female_batch is not None
                        else uplift_f
                    )
                    uplift_m = (
                        _score_uplift_loss(male_scores, male_scores_base, args.uplift_margin)
                        if male_batch is not None
                        else uplift_m
                    )
                    uplift_loss = args.uplift_weight_f * uplift_f + args.uplift_weight_m * uplift_m

                # ===== 7th 핵심: 비대칭 fairness 스케일링 (유지) =====
                fairness_loss = args.fair_f_scale * fairness_f + args.fair_m_scale * fairness_m
                
                # ===== Score Alignment Losses =====
                align_src, align_tgt, gap_value, align_dir = _select_alignment_pair(
                    female_scores, male_scores, args.align_mode
                )
                if align_src is not None:
                    # 1. Wasserstein Loss (7th 유지)
                    wasserstein_loss = _wasserstein_1d(align_src, align_tgt)
                    # 2. Quantile Matching Loss (9th 신규)
                    quantile_loss = _quantile_matching_loss(
                        align_src, align_tgt, args.quantile_levels
                    )
                    # 3. Score Gap Penalty (9th 신규)
                    gap_loss = _score_gap_penalty(align_src, align_tgt)
                    gap_scale = _gap_scale(gap_value, args.gap_scale_temp)
                    wasserstein_loss = wasserstein_loss * gap_scale
                    quantile_loss = quantile_loss * gap_scale
                    gap_loss = gap_loss * gap_scale
                ...
                total_g = (
                    ...
                    + args.lambda_det_guard * det_guard
                    + args.lambda_uplift * uplift_loss
                )
            ...
            metrics_logger.update(
                ...
                g_det=det_loss.item(),
                g_det_guard=det_guard.item(),
                g_uplift=uplift_loss.item(),
                g_uplift_f=uplift_f.item(),
                g_uplift_m=uplift_m.item(),
                g_total=total_g.item(),
                g_w=wasserstein_loss.item(),
                g_q=quantile_loss.item(),
                g_gap=gap_loss.item(),
                gap_value=gap_value.item(),
                gap_scale=gap_scale.item(),
                align_dir=align_dir.item(),
                eps=current_eps,
                beta=current_beta,
                lr_g=current_lr_g,
                lr_d=current_lr_d,
        ...
        if utils.is_main_process():
            log_entry = {
                ...
                "g_det_guard": metrics_logger.meters["g_det_guard"].global_avg,
                "g_uplift": metrics_logger.meters["g_uplift"].global_avg,
                "g_uplift_f": metrics_logger.meters["g_uplift_f"].global_avg,
                "g_uplift_m": metrics_logger.meters["g_uplift_m"].global_avg,
                ...
                "gap_value": metrics_logger.meters["gap_value"].global_avg,
                "gap_scale": metrics_logger.meters["gap_scale"].global_avg,
                "align_dir": metrics_logger.meters["align_dir"].global_avg,
                "epsilon": current_eps,
                "beta": current_beta,
                "lr_g": current_lr_g,
                "lr_d": current_lr_d,
