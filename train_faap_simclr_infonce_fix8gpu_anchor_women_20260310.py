"""
FAAP Training - Gender-Centered SupCon, Anchor=Female (fix8gpu_anchor_women)

성별 중심 anchor 설정: Female을 anchor로, 같은 quality bin의 Male을 positive로.
"Female feature가 같은 품질의 Male feature를 닮아라"

=============================================================================
fix7 vs fix8 실험 비교:
=============================================================================
fix7: 모든 샘플이 anchor, 성별 무관 (Quality SupCon)
fix8_women: Female만 anchor, Male이 target (Gender-Centered)
fix8_man:   Male만 anchor, Female이 target (역방향 비교)
...
Male은 anchor로 사용하지 않음 → gradient가 Female 개선에 집중
...
def parse_args():
    parser = argparse.ArgumentParser("FAAP fix8 anchor=women")
...
def main():
    ...
    if utils.is_main_process():
        ...
        print("=" * 70)
        print("Gender-Centered SupCon — Anchor=FEMALE (fix8gpu_anchor_women)")
        print("=" * 70)
        print("[Anchor 설정]")
        print("  Anchor:   Female (변해야 하는 쪽)")
        print("  Positive: 같은 quality bin의 Male (목표)")
        print("  Negative: 다른 quality bin의 모든 샘플")
        print("  → Female feature가 같은 품질의 Male feature를 닮아가도록 학습")
        print("-" * 70)
    ...
    if utils.is_main_process():
        print(f"\n{'='*70}")
        print(f"Training Complete — Anchor=FEMALE (fix8gpu_anchor_women)")
