...
def parse_args() -> argparse.Namespace:  # 커맨드라인 인자를 파싱해 Namespace로 반환
    parser = argparse.ArgumentParser("FAAP-style training for DETR (8th version - based on 7th success)", add_help=True)
    ...
    parser.add_argument("--epochs", type=int, default=24)  # 학습 에폭 수 (7th와 동일)
    ...
    parser.add_argument("--epsilon_hold_epochs", type=int, default=6, help="epochs to keep epsilon_final before cooldown")
    ...
    parser.add_argument("--beta", type=float, default=0.5, help="detection-preserving loss weight start")
    parser.add_argument("--beta_final", type=float, default=0.6, help="final detection-preserving loss weight")
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="male fairness scaling factor (7th 핵심: 남성 perturbation 억제)",
    )
    ...
    # ===== 8th 신규 개선: 강화된 Wasserstein (후반부) =====
    parser.add_argument(
        "--lambda_w_boost_epoch",
        type=int,
        default=16,
        help="epoch from which to boost lambda_w",
    )
    parser.add_argument(
        "--lambda_w_boost",
        type=float,
        default=0.3,
        help="boosted lambda_w value after boost_epoch",
    )
...
def _scheduled_epsilon(
    ...
    """7th와 동일한 epsilon 스케줄: warmup → hold → cooldown"""
    if warmup_epochs <= 1:
        warmup_end = 0
    else:
...
def _scheduled_lambda_w(epoch: int, boost_epoch: int, lambda_w_base: float, lambda_w_boost: float) -> float:
    """8th 신규: 후반부 lambda_w 부스트 (단순 step 증가)"""
    if epoch >= boost_epoch:
        return lambda_w_boost
    return lambda_w_base
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # ===== 8th 신규: 후반부 lambda_w 부스트 =====
        current_lambda_w = _scheduled_lambda_w(
            epoch, args.lambda_w_boost_epoch, args.lambda_w, args.lambda_w_boost
        )
        ...
        for samples, targets, genders in metrics_logger.log_every(train_loader, args.log_every, f"Epoch {epoch}"):
            ...
            if female_batch is not None or male_batch is not None:
                ...
                total_g = (
                    ...
                    + current_lambda_w * wasserstein_loss
                )
