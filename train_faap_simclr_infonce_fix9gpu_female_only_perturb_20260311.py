"""
FAAP Training - Female-Only Perturbation (fix9gpu_female_only_perturb)

핵심 변경 (fix8 anchor_women 대비):
=============================================================================
1. Female 이미지에만 perturbation 적용, Male은 clean pass-through
2. Generator는 전체 배치에 forward (DDP safe + BN 안정)
3. Male delta를 mask 곱셈으로 zero-out (autograd safe)
4. Feature cosine similarity 메트릭 추가
=============================================================================

설계 근거:
- "남성 데이터 특성을 가지는 여성 데이터"를 만들기 위한 perturbation
- Male 이미지는 clean reference target → 안정적 학습 신호
- Generator 전체 배치 forward → BatchNorm 통계 안정 + DDP gradient sync 보장

=============================================================================
Anchor 설정 (fix8 anchor_women과 동일):
=============================================================================
Anchor:   Female 샘플 (quality bin Q)
Positive: Male 샘플 (같은 quality bin Q)
Negative: 모든 샘플 (다른 quality bin)

차이점: perturbation이 Female에만 적용되므로,
  - Female features = perturbed image에서 추출
  - Male features = clean image에서 추출 (안정적 target)
...
# =============================================================================
# Gender-Centered SupCon Loss (Anchor = Female)
# =============================================================================
...
class GenderAnchorSupConLoss(nn.Module):
    ...
    Positive: 같은 quality bin의 Male
    Negative: 다른 quality bin의 모든 샘플

    Female feature → 같은 품질의 Male feature 방향으로 당김.
    Male은 anchor가 아니므로 직접적인 contrastive gradient를 받지 않음.
    """
    ...
    def forward(
        ...
        # Similarity: anchor(Female) vs 전체
...
def parse_args():
    parser = argparse.ArgumentParser("FAAP fix9 female-only perturbation")
    ...
    parser.add_argument("--batch_size", type=int, default=8)  # fix8: 5 → fix9: 8 (성별 균형 확률 향상)
...
def main():
    ...
    if utils.is_main_process():
        ...
        print("=" * 70)
        print("Female-Only Perturbation — fix9gpu_female_only_perturb")
        print("=" * 70)
        print("[핵심 변경: fix8 anchor_women 대비]")
        print("  1. Female 이미지에만 perturbation 적용")
        print("  2. Male 이미지는 clean pass-through (안정적 reference)")
        print("  3. Generator 전체 배치 forward + male delta mask zero-out")
        print("-" * 70)
        print("[Anchor 설정]")
        print("  Anchor:   Female (perturbed)")
        print("  Positive: 같은 quality bin의 Male (clean)")
        print("  Negative: 다른 quality bin의 모든 샘플")
        print("  → Perturbed-Female feature가 Clean-Male feature를 닮아가도록 학습")
        print("-" * 70)
    ...
    # DDP에서 balance_genders는 무시됨 (DistributedSampler가 우선)
    # batch_size=8로 양 성별 포함 확률을 통계적으로 보장
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for samples, targets, genders in metrics_logger.log_every(train_loader, args.log_every, f"Epoch {epoch}"):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                # ============================================================
                # [fix9 핵심] Female-Only Perturbation
                # Generator를 전체 배치에 forward (BN 안정 + DDP safe)
                # Male delta를 mask 곱셈으로 zero-out (autograd safe)
                # ============================================================
                delta = generator(tensors)

                # Out-of-place mask 곱셈: male delta → 0, female delta → 유지
                gender_mask = torch.ones(
                    delta.shape[0], 1, 1, 1,
                    device=delta.device, dtype=delta.dtype,
                )
                if len(male_idx) > 0:
                    gender_mask[male_idx] = 0.0
                delta = delta * gender_mask
            ...
            # ============================================================
            # [fix9 변경] Delta 메트릭: female 전용
            # ============================================================
            with torch.no_grad():
                if len(female_idx) > 0:
                    dv_f = delta[female_idx]
                    delta_linf = dv_f.abs().amax(dim=(1, 2, 3)).mean()
                    delta_l2 = dv_f.flatten(1).norm(p=2, dim=1).mean()
                else:
                    delta_linf = delta.new_tensor(0.0)
                    delta_l2 = delta.new_tensor(0.0)

                # [fix9 추가] Feature cosine similarity (perturbed-female vs clean-male)
                feat_cos_sim = 0.0
                if len(female_idx) > 0 and len(male_idx) > 0:
                    f_feat = F.normalize(features[female_idx].mean(dim=1), dim=-1)
                    m_feat = F.normalize(features[male_idx].mean(dim=1), dim=-1)
                    feat_cos_sim = torch.mm(f_feat, m_feat.t()).mean().item()
            ...
            metrics_logger.update(
                ...
                feat_cos_sim=feat_cos_sim,  # [fix9 추가]
            )
        ...
        if utils.is_main_process():
            ...
            avg_cos_sim = metrics_logger.meters["feat_cos_sim"].global_avg
            log_entry = {
                "epoch": epoch,
                ...
                "feat_cos_sim": avg_cos_sim,  # [fix9 추가]
            ...
            print(f"\n[Epoch {epoch}] Female-Only Perturbation (fix9)")
            ...
            print(f"  Feature Cos Sim (F↔M): {avg_cos_sim:.4f}")  # [fix9 추가]
            print(f"  Delta (female-only): Linf={log_entry['delta_linf']:.4f}, L2={log_entry['delta_l2']:.4f}")
            print(f"  Epsilon: {current_epsilon:.4f}, LR: {current_lr:.6f}")
            print(f"  Batch gender (avg): F={log_entry['n_f_avg']:.1f}, M={log_entry['n_m_avg']:.1f}")
            if is_best:
    ...
    if utils.is_main_process():
        print(f"\n{'='*70}")
        print(f"Training Complete — Female-Only Perturbation (fix9)")
